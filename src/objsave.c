...
int Crash_offer_rent(struct char_data *ch, struct char_data *recep, int display,
                     int factor);
...
void Crash_report_rent(struct char_data *ch, struct char_data *recep,
                       struct obj_data *obj, long *cost, long *nitems, int display,
                       int factor);
...
int gen_receptionist(struct char_data *ch, struct char_data *recep, int cmd, char *arg,
                     int mode);
...
int Crash_clean_file(char *name)
{
    ...
    if ((rent.rentcode == RENT_CRASH) || (rent.rentcode == RENT_FORCED) ||
...
int Crash_load(struct char_data *ch)
{
    ...
    default:
        mudlog(BRF, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE,
               "SYSERR: %s entering game with undefined rent code %d.", GET_NAME(ch),
               rent.rentcode);
        break;
    }
    ...
    mudlog(NRM, MAX(GET_INVIS_LEV(ch), LVL_GOD), TRUE,
           "%s (level %d) has %d object%s (max %d).", GET_NAME(ch), GET_LEVEL(ch),
           num_objs, num_objs != 1 ? "s" : "", max_obj_save);
                     ...
                     int factor)
{
    ...
    if (display)
    {
        ...
        {
            act("$n tells you, '...which I see you can't afford.'", FALSE, recep, 0, ch,
                TO_VICT);
            return (0);
        }
                     ...
                     int mode)
{
    ...
    if (CMD_IS("rent"))
    {
        ...
        {
            snprintf(buf, sizeof(buf),
                     "$n tells you, 'Rent will cost you %d gold coins per day.'", cost);
        }
        else if (mode == CRYO_FACTOR)
        {
            snprintf(buf, sizeof(buf),
                     "$n tells you, 'It will cost you %d gold coins to be frozen.'",
                     cost);
        }
        act(buf, FALSE, recep, 0, ch, TO_VICT);
        ...
        {            /* cryo */
            ...
            mudlog(NRM, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE, "%s has cryo-rented.",
                   GET_NAME(ch));
            SET_BIT(PLR_FLAGS(ch), PLR_CRYO);
        }
