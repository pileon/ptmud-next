...
/* pool of large output buffers */
int                    buf_largecount   = 0;
/* # of large buffers which exist */
int                    buf_overflows    = 0;
/* # of overflows of output */
int                    buf_switches     = 0;
/* # of switches from small to large buf */
...
int                    scheck           = 0;
/* for syntax checking mode */
struct timeval         null_time;
/* zero-valued time structure */
byte                   reread_wizlist;
/* signal: SIGUSR1 */
byte                   emergency_unban;
/* signal: SIGUSR2 */
FILE                   *logfile         = NULL;
...
#define OPEN_MAX FOPEN_MAX
    ...
    if (max_descs <= 0)
    {
        log("SYSERR: Non-positive max player limit!  (Set at %d using %s).", max_descs,
            method);
        exit(1);
    }
...
int process_output(struct descriptor_data *t)
{
    ...
    strcpy(osb,
           t->output);    /* strcpy: OK (t->output:LARGE_BUFSIZE < osb:MAX_SOCK_BUF-2) */
    ...
    while (nl_pos != NULL)
    {
        ...
        *write_point = '\0';
        ...
        else if (*tmp == '!' && *(tmp + 1))
        {
            ...
            int  starting_pos = t->history_pos, cnt = (t->history_pos == 0 ?
                                                       HISTORY_SIZE - 1 : t->history_pos -
                                                                          1);
...
int perform_subst(struct descriptor_data *t, char *orig, char *subst)
{
    ...
    *(second++) = '\0';
    ...
    strncpy(newsub, orig, strpos -
                          orig);    /* strncpy: OK (newsub:MAX_INPUT_LENGTH+5 > orig:MAX_INPUT_LENGTH) */
