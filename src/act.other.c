    ...
    else
    {
        ...
        if (AFF_FLAGGED(k, AFF_GROUP))
        {
            snprintf(buf, sizeof(buf),
                     "     [%3dH %3dM %3dV] [%2d %s] $N (Head of group)", GET_HIT(k),
        ...
        for (f = k->followers; f; f = f->next)
        {
            ...
            snprintf(buf, sizeof(buf), "     [%3dH %3dM %3dV] [%2d %s] $N",
...
ACMD(do_group)
{
    ...
    if (ch->master)
    {
        act("You can not enroll group members without being head of a group.", FALSE, ch,
            0, 0, TO_CHAR);
        return;
    }
...
ACMD(do_report)
{
    ...
    snprintf(buf, sizeof(buf), "$n reports: %d/%dH, %d/%dM, %d/%dV\r\n", GET_HIT(ch),
             GET_MAX_HIT(ch), GET_MANA(ch), GET_MAX_MANA(ch), GET_MOVE(ch),
             GET_MAX_MOVE(ch));
...
ACMD(do_split)
{
    ...
    if (is_number(buf))
    {
        ...
        if (AFF_FLAGGED(k, AFF_GROUP) && IN_ROOM(k) == IN_ROOM(ch) && !IS_NPC(k) &&
            k != ch)
        {
            GET_GOLD(k) += share;
