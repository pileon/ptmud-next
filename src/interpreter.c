...
void perform_complex_alias(struct txt_q *input_q, char *orig, struct alias_data *a)
{
    ...
    strcpy(buf2,
           orig);    /* strcpy: OK (orig:MAX_INPUT_LENGTH < buf2:MAX_RAW_INPUT_LENGTH) */
    temp = strtok(buf2, " ");
  ...
  */
    ...
    case USURP:
        ...
        act("$n suddenly keels over in pain, surrounded by a white aura...\r\n"
                    "$n's body has been taken over by a new spirit!", TRUE, d->character,
            0, 0, TO_ROOM);
        mudlog(NRM, MAX(LVL_IMMORT, GET_INVIS_LEV(d->character)), TRUE,
...
void nanny(struct descriptor_data *d, char *arg)
{
    ...
    case CON_QCLASS:
        ...
        mudlog(NRM, LVL_IMMORT, TRUE, "%s [%s] new player.", GET_NAME(d->character),
               d->host);
        break;
    ...
    case CON_DELCNF2:
        if (!strcmp(arg, "yes") || !strcmp(arg, "YES"))
        {
            ...
            write_to_output(d, "Character '%s' deleted!\r\n"
                    "Goodbye.\r\n", GET_NAME(d->character));
            mudlog(NRM, LVL_GOD, TRUE, "%s (lev %d) has self-deleted.",
