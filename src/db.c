...
struct help_index_element *help_table  = 0;
/* the help table	 */
int                       top_of_helpt = 0;
...
/* local functions */
int check_bitvector_names(bitvector_t bits, size_t namecount, const char *whatami,
                          const char *whatbits);
...
void free_text_files(void)
{
    char **textfiles[] = { &wizlist, &immlist, &news, &credits, &motd, &imotd, &help,
                           &info, &policies, &handbook, &background, &GREETINGS, NULL };
    int  rf;
...
void index_boot(int mode)
{
    ...
    case DB_BOOT_MOB:
        ...
        log("   %d mobs, %d bytes in index, %d bytes in prototypes.", rec_count, size[0],
            size[1]);
        break;
    case DB_BOOT_OBJ:
        ...
        log("   %d objs, %d bytes in index, %d bytes in prototypes.", rec_count, size[0],
            size[1]);
        break;
    case DB_BOOT_ZON:
...
void discrete_load(FILE *fl, int mode, char *filename)
{
    ...
    for (; ;)
    {
        ...
        {
            log("SYSERR: Format error in %s file %s near %s #%d", modes[mode], filename,
                modes[mode], nr);
...
void parse_room(FILE *fl, int virtual_nr)
{
    ...
    snprintf(buf, sizeof(buf), "SYSERR: Format error in room #%d (expecting D/E/S)",
             virtual_nr);
...
void parse_simple_mob(FILE *mob_f, int i, int nr)
{
    ...
    if (sscanf(line, " %d %d %d %dd%d+%d %dd%d+%d ", t, t + 1, t + 2, t + 3, t + 4, t + 5,
               t + 6, t + 7, t + 8) != 9)
    {
...
void parse_mobile(FILE *mob_f, int nr)
{
    ...
    check_bitvector_names(AFF_FLAGS(mob_proto + i), affected_bits_count, buf2,
                          "mobile affect");
...
void load_zones(FILE *fl, char *zonename)
{
    ...
    for (; ;)
    {
        ...
        {
            if (sscanf(ptr, " %d %d %d %d ", &tmp, &ZCMD.arg1, &ZCMD.arg2, &ZCMD.arg3) !=
                4)
            {
                error = 1;
            }
...
void zone_update(void)
{
    ...
    for (update_u = reset_q.head; update_u; update_u = update_u->next)
    {
            ...
            is_empty(update_u->zone_to_reset))
        {
            ...
            mudlog(CMP, LVL_GOD, FALSE, "Auto zone reset: %s",
...
void save_char(struct char_data *ch)
{
    ...
    strncpy(st.host, ch->desc->host,
            HOST_LENGTH);    /* strncpy: OK (s.host:HOST_LENGTH+1) */
...
void store_to_char(struct char_file_u *st, struct char_data *ch)
{
    ...
    if (!AFF_FLAGGED(ch, AFF_POISON) && time(0) - st->last_logon >= SECS_PER_REAL_HOUR)
    {
...
void char_to_store(struct char_data *ch, struct char_file_u *st)
{
    ...
    if (ch->player.description)
    {
        ...
        strcpy(st->description,
               ch->player.description);    /* strcpy: OK (checked above) */
    }
    else
    {
        *st->description = '\0';
    ...
    strcpy(st->pwd,
           GET_PASSWD(ch));    /* strcpy: OK (that's what GET_PASSWD came from) */
...
int check_object(struct obj_data *obj)
{
    ...
    error |= check_bitvector_names(GET_OBJ_WEAR(obj), wear_bits_count, objname,
                                   "object wear");
    error |= check_bitvector_names(GET_OBJ_EXTRA(obj), extra_bits_count, objname,
                                   "object extra");
    error |= check_bitvector_names(GET_OBJ_AFFECT(obj), affected_bits_count, objname,
                                   "object affect");
                          ...
                          const char *whatbits)
{
    ...
    for (flagnum = namecount; flagnum < sizeof(bitvector_t) * 8; flagnum++)
    {
        if ((1 << flagnum) & bits)
        {
            log("SYSERR: %s has unknown %s flag, bit %d (0 through %zu known).", whatami,
                whatbits, flagnum, namecount - 1);
            error = TRUE;
        }
    }
