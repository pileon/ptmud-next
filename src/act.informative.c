...
void list_one_char(struct char_data *i, struct char_data *ch)
{
    const char *positions[] = { " is lying here, dead.",
                                ...
                                " is resting here.", " is sitting here.", "!FIGHTING!",
                                " is standing here." };
    ...
    for (door = 0; door < NUM_OF_DIRS; door++)
    {
        ...
        else
        {
            send_to_char(ch, "%-5s - %s\r\n", dirs[door],
                         ...
                         ? "Too dark to tell." : world[EXIT(ch, door)->to_room].name);
        }
    }
...
void look_in_obj(struct char_data *ch, char *arg)
{
    ...
    else if (!(bits = generic_find(arg, FIND_OBJ_INV | FIND_OBJ_ROOM | FIND_OBJ_EQUIP, ch,
                                   &dummy, &obj)))
    {
...
ACMD(do_toggle)
{
    ...
    send_to_char(ch, "Hit Pnt Display: %-3s    "
                 ...
                 ONOFF(PRF_FLAGGED(ch, PRF_COMPACT)), YESNO(PRF_FLAGGED(ch, PRF_QUEST)),

                 ONOFF(PRF_FLAGGED(ch, PRF_DISPMANA)), ONOFF(PRF_FLAGGED(ch, PRF_NOTELL)),
                 YESNO(!PRF_FLAGGED(ch, PRF_NOREPEAT)),

                 ONOFF(PRF_FLAGGED(ch, PRF_AUTOEXIT)), YESNO(PRF_FLAGGED(ch, PRF_DEAF)),
                 buf2,
...
ACMD(do_commands)
{
    ...
    for (no = 1, cmd_num = 1;
         cmd_info[cmd_sort_info[cmd_num]].command[0] != '\n'; cmd_num++)
    {
