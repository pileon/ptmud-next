...
{
    strncpy(dest, source,
            totalsize - 1);    /* strncpy: OK (we must assume 'totalsize' is correct) */
    dest[totalsize - 1] = '\0';
...
void mudlog(int type, int level, int file, const char *str, ...)
{
    ...
    for (i = descriptor_list; i; i = i->next)
    {
        ...
        send_to_char(i->character, "%s%s%s", CCGRN(i->character, C_NRM), buf,
...
size_t sprintbit(bitvector_t bitvector, const char *names[], char *result, size_t reslen)
{
    ...
    *result = '\0';
    ...
    for (nr = 0; bitvector && len < reslen; bitvector >>= 1)
    {
        ...
        {
            nlen = snprintf(result + len, reslen - len, "%s ",
                            *names[nr] != '\n' ? names[nr] : "UNDEFINED");
