...
void affect_modify(struct char_data *ch, byte loc, sbyte mod, bitvector_t bitv, bool add)
{
    ...
    default:
        log("SYSERR: Unknown apply adjust %d attempt (%s, affect_modify).", loc,
            __FILE__);
        break;
...
void obj_to_char(struct obj_data *object, struct char_data *ch)
{
    if (object && ch)
    {
        ...
        log("SYSERR: NULL obj (%p) or char (%p) passed to obj_to_char.", (void *) object,
            (void *) ch);
}
...
void extract_char_final(struct char_data *ch)
{
    ...
    if (IN_ROOM(ch) == NOWHERE)
    {
        log("SYSERR: NOWHERE extracting char %s. (%s, extract_char_final)", GET_NAME(ch),
            __FILE__);
        exit(1);
    }
...
struct obj_data *create_money(int amount)
{
    ...
    else
    {
        ...
        {
            snprintf(buf, sizeof(buf), "It looks to be about %d coins.",
                     100 * (amount / 100));
        }
