...
void perform_put(struct char_data *ch, struct obj_data *obj, struct obj_data *cont)
{
    ...
    else
    {
        ...
        if (OBJ_FLAGGED(obj, ITEM_NODROP) && !OBJ_FLAGGED(cont, ITEM_NODROP))
        {
            ...
            act("You get a strange feeling as you put $p in $P.", FALSE, ch, obj, cont,
                TO_CHAR);
        }
        else
        {
...
ACMD(do_get)
{
    ...
    else
    {
        ...
        else
        {
            ...
            for (cont                            = world[IN_ROOM(
                    ch)].contents; cont; cont = cont->next_content)
            {
                if (CAN_SEE_OBJ(ch, cont) &&
                    (cont_dotmode == FIND_ALL || isname(arg2, cont->name)))
                {
...
void name_from_drinkcon(struct obj_data *obj)
{
    ...
    if (!obj ||
        (GET_OBJ_TYPE(obj) != ITEM_DRINKCON && GET_OBJ_TYPE(obj) != ITEM_FOUNTAIN))
    {
        return;
    }
...
ACMD(do_pour)
{
    ...
    if (subcmd == SCMD_POUR)
    {
        send_to_char(ch, "You pour the %s into the %s.", drinks[GET_OBJ_VAL(from_obj, 2)],
                     arg2);
    }
    ...
    GET_OBJ_VAL(from_obj, 1) -= (amount = (GET_OBJ_VAL(to_obj, 0) -
...
int find_eq_pos(struct char_data *ch, struct obj_data *obj, char *arg)
{
    ...
    const char *keywords[] = { "!RESERVED!", "finger", "!RESERVED!", "neck", "!RESERVED!",
                               "body", "head", "legs", "feet", "hands", "arms", "shield",
                               "about", "waist", "wrist", "!RESERVED!", "!RESERVED!",
                               "!RESERVED!", "\n" };
