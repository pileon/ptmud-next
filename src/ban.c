...
ACMD(do_ban)
{
    ...
    if (!*argument)
    {
        ...
        send_to_char(ch, BAN_LIST_FORMAT, "Banned Site Name", "Ban Type", "Banned On",
                     "Banned By");
        ...
        for (ban_node = ban_list; ban_node; ban_node = ban_node->next)
        {
            ...
            send_to_char(ch, BAN_LIST_FORMAT, ban_node->site, ban_types[ban_node->type],
...
ACMD(do_unban)
{
    ...
    mudlog(NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE,
           "%s removed the %s-player ban on %s.", GET_NAME(ch), ban_types[ban_node->type],
           ban_node->site);
