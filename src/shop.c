...
int read_list(FILE *shop_f, struct shop_buy_data *list, int new_format, int max,
              int type);
...
int buy_price(struct obj_data *obj, int shop_nr, struct char_data *keeper,
              struct char_data *buyer);

int sell_price(struct obj_data *obj, int shop_nr, struct char_data *keeper,
               struct char_data *seller);

char *list_object(struct obj_data *obj, int cnt, int oindex, int shop_nr,
               ...
               struct char_data *seller)
{
    float sell_cost_modifier = SHOP_SELLPROFIT(shop_nr) *
                               (1 - (GET_CHA(keeper) - GET_CHA(seller)) / (float) 70);
...
void shopping_buy(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
    ...
    snprintf(tempbuf, sizeof(tempbuf), shop_index[shop_nr].message_buy, GET_NAME(ch),
             goldamt);
                                 ...
                                 struct char_data *keeper, int shop_nr, int msg)
{
    ...
    case OBJECT_NOVAL:
        snprintf(buf, sizeof(buf),
                 "%s You've got to be kidding, that thing is worthless!", GET_NAME(ch));
        break;
    case OBJECT_NOTOK:
...
void shopping_sell(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
    ...
    snprintf(tempbuf, sizeof(tempbuf), shop_index[shop_nr].message_sell, GET_NAME(ch),
             goldamt);
    do_tell(keeper, tempbuf, cmd_tell, 0);
...
char *list_object(struct obj_data *obj, int cnt, int aindex, int shop_nr,
                  struct char_data *keeper, struct char_data *ch)
{
    ...
    snprintf(result, sizeof(result), " %2d)  %9s   %-48s %6d\r\n", aindex, quantity,
             itemname, buy_price(obj, shop_nr, keeper, ch));
    return (result);
}
...
void boot_the_shops(FILE *shop_f, char *filename, int rec_count)
{
    ...
    while (!done)
    {
        ...
        {        /* New shop */
            ...
            shop_index[top_shop].no_such_item1 = read_shop_message(0, SHOP_NUM(top_shop),
                                                                   shop_f, buf2);
            shop_index[top_shop].no_such_item2 = read_shop_message(1, SHOP_NUM(top_shop),
                                                                   shop_f, buf2);
            shop_index[top_shop].do_not_buy    = read_shop_message(2, SHOP_NUM(top_shop),
                                                                   shop_f, buf2);
            shop_index[top_shop].missing_cash1 = read_shop_message(3, SHOP_NUM(top_shop),
                                                                   shop_f, buf2);
            shop_index[top_shop].missing_cash2 = read_shop_message(4, SHOP_NUM(top_shop),
                                                                   shop_f, buf2);
            shop_index[top_shop].message_buy   = read_shop_message(5, SHOP_NUM(top_shop),
                                                                   shop_f, buf2);
            shop_index[top_shop].message_sell  = read_shop_message(6, SHOP_NUM(top_shop),
                                                                   shop_f, buf2);
...
char *customer_string(int shop_nr, int detailed)
{
    ...
    while (*trade_letters[sindex] != '\n' && len + 1 < sizeof(buf))
    {
        ...
        {
            buf[len++] = (IS_SET(flag, SHOP_TRADE_WITH(shop_nr)) ? '_'
                                                                 : *trade_letters[sindex]);
            buf[len]   = '\0';
...
void list_detailed_shop(struct char_data *ch, int shop_nr)
{
    ...
    for (sindex = 0; SHOP_BUYTYPE(shop_nr, sindex) != NOTHING; sindex++)
    {
        ...
        linelen = snprintf(buf1, sizeof(buf1), "%s (#%d) [%s]",
                           ...
                           SHOP_BUYWORD(shop_nr, sindex) ? SHOP_BUYWORD(shop_nr, sindex)
                                                         : "all");
