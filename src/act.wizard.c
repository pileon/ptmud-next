...
void do_stat_room(struct char_data *ch)
{
    ...
    for (i = 0; i < NUM_OF_DIRS; i++)
    {
        ...
        send_to_char(ch,
                     ...
                     buf2, rm->dir_option[i]->general_description
...
void do_stat_object(struct char_data *ch, struct obj_data *j)
{
    ...
    send_to_char(ch, "Name: '%s%s%s', Aliases: %s\r\n", CCYEL(ch, C_NRM),
                 j->short_description ? j->short_description : "<None>", CCNRM(ch, C_NRM),
                 j->name);
    ...
    case ITEM_STAFF:
        send_to_char(ch, "Spell: %s at level %d, %d (of %d) charges remaining\r\n",
                     skill_name(GET_OBJ_VAL(j, 3)), GET_OBJ_VAL(j, 0), GET_OBJ_VAL(j, 2),
                     GET_OBJ_VAL(j, 1));
        break;
    case ITEM_WEAPON:
    ...
    for (i = 0; i < MAX_OBJ_AFFECT; i++)
    {
        if (j->affected[i].modifier)
        {
            ...
            send_to_char(ch, "%s %+d to %s", found++ ? "," : "", j->affected[i].modifier,
                         buf);
        }
    }
    if (!found)
...
void do_stat_character(struct char_data *ch, struct char_data *k)
{
    ...
    sprinttype(k->player.chclass, IS_NPC(k) ? npc_class_types : pc_class_types, buf,
               sizeof(buf));
    send_to_char(ch, "%sClass: %s, Lev: [%s%2d%s], XP: [%s%7d%s], Align: [%4d]\r\n",
                 IS_NPC(k) ? "Monster " : "", buf, CCYEL(ch, C_NRM), GET_LEVEL(k),
    ...
    send_to_char(ch, "Str: [%s%d/%d%s]  Int: [%s%d%s]  Wis: [%s%d%s]  "
            "Dex: [%s%d%s]  Con: [%s%d%s]  Cha: [%s%d%s]\r\n", CCCYN(ch, C_NRM),
                 GET_STR(k), GET_ADD(k), CCNRM(ch, C_NRM), CCCYN(ch, C_NRM), GET_INT(k),
                 CCNRM(ch, C_NRM), CCCYN(ch, C_NRM), GET_WIS(k), CCNRM(ch, C_NRM),
                 CCCYN(ch, C_NRM), GET_DEX(k), CCNRM(ch, C_NRM), CCCYN(ch, C_NRM),
                 GET_CON(k), CCNRM(ch, C_NRM), CCCYN(ch, C_NRM), GET_CHA(k),
                 CCNRM(ch, C_NRM));
...
ACMD(do_stat)
{
    ...
    else
    {
        ...
        else if ((object = get_obj_in_list_vis(ch, name, &number,
...
ACMD(do_purge)
{
    ...
    {            /* no argument. clean out the room */
        ...
        act("$n gestures... You are surrounded by scorching flames!", FALSE, ch, 0, 0,
            TO_ROOM);
...
ACMD(do_advance)
{
    ...
    if (newlevel < oldlevel)
        log("(GC) %s demoted %s from level %d to %d.", GET_NAME(ch), GET_NAME(victim),
            oldlevel, newlevel);
    else
...
void perform_immort_invis(struct char_data *ch, int level)
{
    ...
    for (tch = world[IN_ROOM(ch)].people; tch; tch = tch->next_in_room)
    {
        ...
        if (GET_LEVEL(tch) < GET_INVIS_LEV(ch) && GET_LEVEL(tch) >= level)
        {
            act("You suddenly realize that $n is standing beside you.", FALSE, ch, 0, tch,
                TO_VICT);
        }
    }
...
ACMD(do_force)
{
    ...
    char                   arg[MAX_INPUT_LENGTH], to_force[MAX_INPUT_LENGTH], buf1[
                                   MAX_INPUT_LENGTH + 32];
    ...
    else if ((GET_LEVEL(ch) < LVL_GRGOD) || (str_cmp("all", arg) && str_cmp("room", arg)))
    {
        ...
        else
        {
            ...
            mudlog(NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s forced %s to %s",
...
ACMD(do_wiznet)
{
    char                   buf1[MAX_INPUT_LENGTH + MAX_NAME_LENGTH + 32], buf2[
...
ACMD(do_wizutil)
{
    ...
    else
    {
        ...
        case SCMD_THAW:
            ...
            act("A sudden fireball conjured from nowhere thaws $n!", FALSE, vict, 0, 0,
                TO_ROOM);
            break;
        case SCMD_UNAFFECT:
            if (vict->affected)
...
ACMD(do_show)
{
    ...
    strcpy(arg, two_arguments(argument, field,
    ...
    case 2:
        ...
        send_to_char(ch, "Last: %-20.16s  Played: %3dh %2dm\r\n", ctime(&vbuf.last_logon),
                     vbuf.played / 3600, vbuf.played / 60 % 60);
        break;
    ...
    case 4:
        ...
        send_to_char(ch, "Current stats:\r\n"
                     ...
                     top_of_world + 1, top_of_zone_table + 1, buf_largecount,
...
struct set_struct
{
    ...
    const char pcnpc;
    const char type;
}          set_fields[] = {{ "brief",     LVL_GOD,    PC,   BINARY },  /* 0 */
